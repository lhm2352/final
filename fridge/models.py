...
class Recommendation(models.Model):
    ...
    def has_what(owner, pk):
        ...
        for code in menu_main:
            fridge_item = fridge_items.filter(iteminfo__ingredientCode=code).first()
            ...
            if fridge_item is not None:
                existing_ingredients.append(fridge_item)
                # existing_main_ingredients[Ingredient.objects.get(ingredientCode=code)] = quantity
            else:
                ingre=Ingredient.objects.get(ingredientCode=code)
                missing_ingredients.append(ingre)

                # missing_fridge_item=FridgeItem(iteminfo__ingredientCode=code)
                # missing_ingredients.append(missing_fridge_item)
                # missing_main_ingredients[Ingredient.objects.get(ingredientCode=code)] = quantity

        for code in menu_sub:
            fridge_item = fridge_items.filter(iteminfo__ingredientCode=code).first()
            if fridge_item is not None:
                existing_ingredients.append(fridge_item)
            else:
                ...
                # missing_ingredients.append(fridge_item)
